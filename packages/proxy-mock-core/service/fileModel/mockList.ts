...
export async function getMockConfig(
  ...
  const mockFileUrl = mockConfigJson.sence
    ? path.join(baseUrl, mockConfigJson.sence + '.js')
    : ''
  return {
    mockConfigJson,
    mockFileUrl,
...
export async function updateMock(
  mockDataFileUrl: string,
  mockOpOptions: FileListItem & MockConfigJson,
  sourceFileInfo?: FileListItem,
) {
  ...
  await fs.promises.writeFile(
    indexJsonUrl,
    JSON.stringify(Object.assign(oldMockConfigJson, configJson), null, 2),
  )
}
...
/** 切换是否打开Mock */
export async function toggleMock(
  mockDataFileUrl: string,
  fileInfo: FileListItem,
) {
  const fileName = urlToFileName(fileInfo)
  ...
  const mockConfigJson: MockConfigJson = await readJson(indexJsonUrl)
  mockConfigJson.mock =
    mockConfigJson?.mock === undefined ? true : !mockConfigJson?.mock
  await fs.promises.writeFile(
    indexJsonUrl,
    JSON.stringify(mockConfigJson, null, 2),
  )
}
